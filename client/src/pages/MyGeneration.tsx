...
const MyGeneration = () => {
  ...
  const handleDelete = async (id:string)=>{
    try{
      const confirm = window.confirm('Do you want to delete this Thumbnail?')
      if(!confirm) return;
      ...
      <div className="mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32">
        ...
        {!loading && thumbnails.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-sm text-zinc-400 mt-2">Generated your first thumbnail to see it here.</p>
          </div>
        )}

        {/* GRID */}
        {!loading && thumbnails.length > 0 && (
          <div className="columns-1 sm:columns-2 lg:coolumns-3 2xl:columns-4 gap-8">
              ...
              return (
                <div key={thumb._id} onClick={()=> navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                  ...
                  <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                    ...
                    {thumb.isGenerating && <div className="absolute inset-0 flex items-center justify-center text-sm font-medium text-white bg-black/50">Generating...</div>}
                  </div>
                  ...
                  <div onClick={(e)=>e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                    ...
                    <ArrowUpRightIcon onClick={()=> handleDelete(thumb._id)} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all"/>
                    </Link>
                  </div>
                </div>
              )
